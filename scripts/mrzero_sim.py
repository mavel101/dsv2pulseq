...
if include_sens:
    ...
    maps_cplx = maps_mag * np.exp(1j * maps_phase)
    obj_p.coil_sens = torch.tensor(maps_cplx, dtype=torch.complex64)
...
plt.plot(np.abs(signal_numpy))

plt.figure()
plt.title('Signal real and imaginary parts')
plt.plot(signal_numpy.real, label='real')
plt.plot(signal_numpy.imag, label='imag')
...
if insert_raw:
    ...
    if nc != signal_numpy.shape[1]:
        print(f"Number of coils {nc} in raw data file does not match simulated coils {signal_numpy.shape[0]}.")
    ...
    for k, mdb in enumerate(raw_data[-1]['mdb']):
        if not mdb.is_flag_set('ACQEND') and not mdb.is_flag_set('SYNCDATA') and not mdb.is_flag_set('NOISEADJSCAN'):
            ...
            mdb.data = signal_numpy[sample_ix:sample_ix + mdb_samples,:].T
            raw_data[-1]['mdb'][k] = mdb
            sample_ix += mdb_samples

    if sample_ix != signal_numpy.shape[0]:
        print(f"MDB data length {sample_ix} does not match simulated signal length {signal_numpy.shape[0]}.")
